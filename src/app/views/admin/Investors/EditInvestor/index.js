import {
  ...
  const param = useParams();
    ...
    const updateInvestor = async (data) => {
      ...
      try {
        const res = await AuthServices.updateInvestor(obj);
    ...
    const getInvestorById = async () => {
        ...
        try {
          ...
          const { data } = await AuthServices.getInvestorById( 
      param?.id
          );
          console.log(data?.user)
          setValue("name", data?.user?.name || "");
          setValue("email", data?.user?.email || "");
          setValue("properties", data?.user?.properties || []);
          setSelectedProperties(data?.user?.properties || []);
        } catch (error) {
          ErrorHandler(error);
          console.log(error);
        } finally {
    ...
    return (
      <>
        ...
        <Box
          component="form"
          onSubmit={handleSubmit(updateInvestor)}
          sx={{
            margin: "14px 16px",
            padding: "20px",
