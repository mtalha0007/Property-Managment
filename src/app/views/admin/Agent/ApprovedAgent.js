...
const App = () => {
  ...
  useEffect(() => {
    getAgentList(search, page + 1, limit, "pending");
  }, [page, limit, search]);
  ...
  const handleSearch = () => {
    getAgentList(search, 1, limit, "pending");
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await AuthServices.updateAgentStatus(obj);
